...
fn public_snapshot_api_does_not_require_a_colocated_git_cli_binary() {
    ...
    let status = Command::new(&consumer)
        ...
        .status()
        .expect("run standalone public API consumer");
...
fn dirty_snapshot_rejects_command_bearing_filters_without_launching_them() {
    ...
    git(repo.path(), &["add", "tracked.txt", ".gitattributes"]);
    git(repo.path(), &["commit", "-qm", "add filtered file"]);

    let helper_root = tempfile::TempDir::new().expect("filter helper root");
    ...
    assert!(
        !marker.exists(),
        "snapshot launched a repository filter helper"
    );
    if let Err(error) = result {
        assert_eq!(
            error
                .downcast_ref::<DirtyCheckoutError>()
                .expect("typed filter rejection")
                .kind(),
...
fn adoption_returns_opaque_receipt_and_revocation_is_receipt_bound() {
    ...
    revoke_dirty(repo.path(), state_home.path(), &receipt.receipt_id)
        .expect_err("repeating a receipt-bound revocation must fail");
    assert_eq!(
        git(
            repo.path(),
            &["status", "--porcelain=v1", "--untracked-files=all"]
...
fn concurrent_adoption_consumes_a_challenge_exactly_once() {
    ...
    let winner = results
        ...
        .expect("one winning receipt");
    let loser = results
        .iter()
        .find_map(|result| result.as_ref().err())
        .expect("one losing adoption");
    assert_eq!(
        loser
            .downcast_ref::<DirtyCheckoutError>()
            .expect("typed loser")
...
fn revocation_crash_state_never_restores_a_consumed_challenge() {
    ...
    let pending_path = state_dir.join(format!(
        ...
        lease["adoption"]["challenge_digest"]
            .as_str()
