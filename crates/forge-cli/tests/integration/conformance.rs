...
fn issue_list_label_filter_conforms_across_providers() {
    ...
    // REST shape (`gh api --paginate --slurp repos/<slug>/issues`): pages are
    // wrapped in an outer array, URLs are `html_url`, and the author is `user`.
...
fn issue_list_empty_conforms_across_providers() {
    ...
    let github = github_call(
        // REST empty page under `--slurp` is an outer array wrapping one empty
        // page.
        &gh_issue_list_stub("[[]]", "[[]]"),
