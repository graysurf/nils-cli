...
fn github_review_thread_idempotent_stub(capture: &str, existing_outdated: bool) -> String {
    format!(
        r#"#!/bin/sh
set -eu
printf '%s\n' "$*" >> {capture:?}
...
case "$2" in
  ...
  graphql)
    case "$*" in
      *"reviewThreads(first: 100)"*)
        printf '%s\n' '{{"data":{{"repository":{{"pullRequest":{{"reviewThreads":{{"nodes":[{{"id":"PRRT_existing","isResolved":false,"isOutdated":{existing_outdated},"path":"src/lib.rs","comments":{{"nodes":[{{"author":{{"login":"quality-bot"}},"body":"Duplicate finding body.","createdAt":"2026-07-14T04:00:00Z","url":"https://github.com/acme/widgets/pull/44#discussion_r1"}}]}}}}]}}}}}}}}}}'
        ;;
      *"reviewThreads(first: 100)"*)
        printf '%s\n' '{{"data":{{"repository":{{"pullRequest":{{"reviewThreads":{{"nodes":[]}}}}}}}}}}'
        ;;
