...
use plan_issue::lifecycle_vnext::visible_lint::{LintHints, lint_visible};
...
fn lifecycle_vnext_render_validation_visible_body_passes_lint() {
    ...
    let payload = write_payload(
        ...
        json!({
            "overall": "pass",
            "commands": [
                {"command": "cargo test -p nils-plan-issue lifecycle_vnext_render", "status": "pass", "evidence": "ci.log"}
            ],
            "waivers": []
        }),
    );
