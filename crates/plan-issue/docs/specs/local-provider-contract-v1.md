...
- `plan-issue` local routing — parameterizing the hardcoded
  `--provider gitlab` base args at
  `crates/plan-issue/src/forge_cli_adapter.rs:127` (Task 2.2).
...
- **`forge-cli` `Provider::Local`** (Task 2.1) implements this store: the Half A
  ...
  lives in `forge-cli`, riding the rail `plan-issue` already uses for GitLab
  via `ForgeCliAdapter`.
- **`plan-issue` local routing** (Task 2.2) is a small change: parameterize
  the hardcoded `--provider gitlab` base args at
  `crates/plan-issue/src/forge_cli_adapter.rs:127` so the same forge-routed
