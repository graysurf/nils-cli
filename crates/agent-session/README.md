...
- `POST /sessions` normally creates a fresh session from `agent`, optional `cwd`, `title`, `id`, `prompt`, and
  ...
  `provider_resume` metadata, and starts tmux with the canonical resume command. In resume-id mode, omit `cwd` and
  `prompt`; invalid, missing, ambiguous, or unsupported provider ids return structured errors.
