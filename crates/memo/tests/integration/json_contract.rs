...
fn json_contract() {
    ...
    assert_eq!(add_json["schema_version"], "cli.memo.add.v1");
    ...
    assert_eq!(update_json["schema_version"], "cli.memo.update.v1");
    assert_eq!(update_json["ok"], true);
    assert_eq!(update_json["data"]["state"], "pending");

    let list_output = run_memo(&db_path, &["--json", "list", "--limit", "20"], None);
    assert_eq!(
        list_output.code,
        0,
        "list failed: {}",
    ...
    assert_eq!(list_json["schema_version"], "cli.memo.list.v1");
    ...
    let search_output = run_memo(&db_path, &["--json", "search", "ssd", "--limit", "5"], None);
    assert_eq!(
        search_output.code,
        0,
        "search failed: {}",
    ...
    assert_eq!(search_json["schema_version"], "cli.memo.search.v1");
    ...
    let fetch_output = run_memo(&db_path, &["--json", "fetch", "--limit", "1"], None);
    assert_eq!(
        fetch_output.code,
        0,
        "fetch failed: {}",
    ...
    assert_eq!(fetch_json["schema_version"], "cli.memo.fetch.v1");
    assert!(
    ...
    let delete_without_yes = run_memo(&db_path, &["--json", "delete", item_id], None);
    assert_eq!(
    ...
    let delete_output = run_memo(&db_path, &["--json", "delete", item_id, "--yes"], None);
    assert_eq!(
        delete_output.code,
        0,
        "delete failed: {}",
    ...
    assert_eq!(delete_json["schema_version"], "cli.memo.delete.v1");
