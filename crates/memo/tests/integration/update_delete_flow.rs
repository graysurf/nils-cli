...
fn update_and_delete_keep_layers_consistent() {
    ...
    let fetch_after_update = run_memo(&db_path, &["--json", "fetch", "--limit", "20"], None);
    assert_eq!(
    ...
    let delete_without_yes = run_memo(&db_path, &["delete", item_id_str], None);
    ...
    let delete_with_compat_hard = run_memo(&db_path, &["delete", item_id_str, "--hard"], None);
    ...
    let delete_output = run_memo(&db_path, &["--json", "delete", item_id_str, "--yes"], None);
    assert_eq!(
        delete_output.code,
        0,
        "delete failed: {}",
    ...
    let list_after_delete = run_memo(&db_path, &["--json", "list", "--limit", "20"], None);
    assert_eq!(
