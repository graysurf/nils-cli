...
| `glab` `mr note create --resolvable` | `forge-cli pr review` on GitLab posts a non-resolvable status note when this flag is present (probed via `--help`); on older `glab` it falls back to the bare `glab mr note <id>` form, which stays resolvable and can register on the `pr merge` thread gate | `brew upgrade glab` |
