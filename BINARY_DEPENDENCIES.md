...
| `/usr/bin/systemd-run`, `/usr/bin/systemctl` | `agent-hook finish-line` Linux activation, transient-user-cgroup execution, exact runner/config/control FD delivery through `OpenFile`, bounded cancellation, and quiescent-unit/cgroup inspection; both fixed paths must be root-owned, executable, non-symlink regular files that are not group/world-writable. `/usr/bin/systemctl` also supports the fail-closed `main-agent worker cancel` proof described below | `open` requires trusted binaries, procfs, cgroup v2, unprivileged user namespaces, and a working user manager. Execution additionally requires memfd sealing, non-dumpable runner/supervisor isolation, pidfd, and systemd `OpenFile` support. Missing or untrusted containment fails closed; non-Linux activation is unsupported | Normally provided by the systemd package |
...
| `lipo`, `codesign`, `spctl` | Architecture, exact signing-identity, CLI notarization assessment, and mandatory app Gatekeeper/notary verification for the locked Peekaboo v4.2.2 backend; the exact v3.9.3 tuple is recognized only to authenticate an in-place upgrade | Required on macOS for strict install/verify/doctor | Preinstalled with macOS/Xcode command-line tools |
