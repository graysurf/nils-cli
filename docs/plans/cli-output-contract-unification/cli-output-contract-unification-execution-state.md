...
- Status: complete (Sprint 1 + Sprint 2 + Sprint 3 all delivered)
- Target scope: whole plan
- Execution window: whole plan — Task 3.4 lands the final lint script gate
- Staged execution confirmation: confirmed(Sprint 1 PR #375, Sprint 2 PR #376,
  Task 3.1 PR #377, Task 3.2 PR #378, Task 3.3 PR #380 merged; Task 3.4 in this PR)
- Current task: Task 3.4 complete
- Next task: none — plan delivered end to end
- Last updated: 2026-05-20
- Branch/commit: feat/cli-output-contract-lint-3-4 (pending push)
- Source document:
...
| Task 3.4 | done   | Workspace lint script for legacy contract usage                   | scripts/ci/cli-output-contract-lint.sh + scripts/ci/tests/cli-output-contract-lint.test.sh; nils-cli-verify-required-checks.sh wires it into docs-only; DEVELOPMENT.md documents it                                                                   | 3 checks: (a) new `--json` bool without `hide = true`, (b) inline `process::exit(1|2)` in `main.rs`, (c) camelCase serde rename outside the documented alias allowlist; self-test exercises all three regression classes against synthetic fixtures; allowlists track grandfathered codex-cli / gemini-cli / image-processing / plan-issue-cli `--json` flags + AWP / api-test / api-testing-core / staged-context camelCase sites |
...
| `bash scripts/ci/nils-cli-checks-entrypoint.sh --docs-only` | pass | docs hygiene + plan-bundle validate + cli-output-contract lint green | terminal log |
...
| `bash scripts/ci/cli-output-contract-lint.sh` | pass | clean baseline on post-migration workspace | terminal log |
| `bash scripts/ci/tests/cli-output-contract-lint.test.sh` | pass | baseline + 3 synthetic regressions ((a)(b)(c)) all fire | terminal log |
...
### 2026-05-20 (Task 3.4 → feat/cli-output-contract-lint-3-4)
...
- Read:
  ...
  - scripts/ci/docs-hygiene-audit.sh (style reference)
  - DEVELOPMENT.md
  - workspace survey: `rg rename_all = "camelCase"`, `rg long = "json"`,
    `rg process::exit\(1|2\)` to seed allowlists
- Changed:
  - scripts/ci/cli-output-contract-lint.sh (new — 3 checks: (a) `--json`
    bool without `hide = true`, (b) inline `process::exit(1|2)` in
    `crates/*/src/main.rs`, (c) camelCase serde rename outside documented
    alias allowlist; JSON_ALLOWED_FILES seeds grandfathered codex-cli,
    gemini-cli, image-processing, plan-issue-cli `--json` flags;
    CAMEL_ALLOWED_FILES seeds AWP records, api-test/api-testing-core JUnit
    schemas, semantic-commit staged-context v2 aliases)
  - scripts/ci/tests/cli-output-contract-lint.test.sh (new self-test that
    creates temp git worktrees, runs the lint against a clean baseline +
    three synthetic regressions, asserts each fires)
  - .agents/skills/nils-cli-verify-required-checks/scripts/nils-cli-verify-required-checks.sh
    (wires lint into both full + docs-only check sequences)
  - DEVELOPMENT.md (docs lint in the required-checks list + a `§3.1`
    callout pointing at the lint and its self-test)
- Validated:
  - `bash scripts/ci/cli-output-contract-lint.sh` (clean — strict=0,
    errors=0)
  - `bash scripts/ci/cli-output-contract-lint.sh --strict` (clean — strict=1,
    errors=0)
  - `bash scripts/ci/tests/cli-output-contract-lint.test.sh` (baseline +
    3 regressions all fire as expected)
  - `bash scripts/ci/nils-cli-checks-entrypoint.sh --docs-only` (PASS,
    includes the new lint)
- Blocked by: none
- Next: open feature PR with `/deliver-feature-pr`; plan is complete after
  this PR merges.
